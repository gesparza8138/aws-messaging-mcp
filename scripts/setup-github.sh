...
step "Variables and secrets"
# Public repo: Actions logs are public, and GitHub masks secrets but NOT
# variables. Anything embedding the AWS account ID or the owner's phone
# number therefore goes into secrets.
gh variable set AWS_REGION --repo "$FULL" --body "$AWS_REGION"
gh secret set AWS_ACCOUNT_ID  --repo "$FULL" --body "$AWS_ACCOUNT_ID"
