...
| R8 | CloudFront Free plan terms change, or CloudFormation cannot attach a distribution to a flat-rate plan (console shows plans as chosen at creation; verified available in the target account on 2026-08-22, one existing PAYG distribution) | Low–Medium | ~$6 / month, or a manual console step after deploy | Verify `AWS::CloudFront::Distribution` support in M1; otherwise document the one-time console enrolment and keep WAF on PAYG until done |
...
| 2026-08-22 | `WWW-Authenticate` on 401s (A4) is restored at the edge by a CloudFront Function on viewer-response | Lambda Function URLs rename it to `x-amzn-Remapped-www-authenticate` on every response (platform behaviour, also true of API Gateway); a CloudFront Function is free on the flat-rate plan and keeps the Function-URL origin design |
