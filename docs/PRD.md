...
| Repository | `github.com/gesparza8138/aws-messaging-mcp` (private) |
| Last updated | 2026-08-22 |
| AWS account | `585008049602` (Gabes-Dev-Account-2), dev and prod stacks side by side |
...
4. **Return the AWS response verbatim** (`MessageId` etc.) plus a `ServerMetadata` block (guardrail decisions, cost estimate if available).
...
| R2 | Cognito discovery metadata omits `code_challenge_methods_supported`; a strict client may refuse PKCE | Low | Auth fails | Verify with both clients in milestone M1; fallback is to front Cognito's metadata with a small Lambda/CloudFront Function that adds the field |
| R3 | CloudFront buffering interferes with Streamable HTTP | Low | Broken responses | Stateless JSON responses only; `InvokeMode: BUFFERED`; test early |
...
| R7 | Claude Code home IP changes break WAF allow-list | Medium | Local dev blocked | Document `scripts/update-my-ip.sh`; or accept token-only for Claude Code by allowing `0.0.0.0/0` on a rate-limited second rule — decide in M1 |
| R8 | CloudFront Free plan terms change, or CloudFormation cannot attach a distribution to a flat-rate plan (console shows plans as chosen at creation; verified available in account `585008049602` on 2026-08-22, one existing PAYG distribution) | Low–Medium | ~$6 / month, or a manual console step after deploy | Verify `AWS::CloudFront::Distribution` support in M1; otherwise document the one-time console enrolment and keep WAF on PAYG until done |
...
| R10 | Large downloads exceed the Free plan's 100 GB / month | Low | Overage is blocked, not billed, on flat-rate plans — downloads fail | Bucket quota guardrail; CloudWatch alarm on `BytesDownloaded` at 80 GB |
...
| 2026-08-22 | Account `585008049602` for both stages; delegate `mcp.gabriel-esparza.com` from GoDaddy to Route 53 rather than moving the domain | Keeps existing GoDaddy DNS/email untouched; Route 53 zone lets CloudFormation manage certs and records |
