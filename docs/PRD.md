...
4. **Return the AWS response verbatim** (`MessageId` etc.) plus a `ServerMetadata` block (guardrail decisions, cost estimate if available). `ServerMetadata` may also carry `content_digests` — a SHA-256 and decoded byte count per binary part the server received — so a caller can verify a payload survived transit without reading the echoed bytes back. `ServerMetadata` is an **open envelope**: its output schema permits additional properties and changes to it are **additive-only**, because clients cache tool schemas and a response failing a cached schema is discarded whole — a successful send then reads to the caller as an error (Appendix C, 2026-09-01).
...
7. **File tools are separate from messaging media.** Nothing uploaded for MMS/RCS is ever exposed through CloudFront, and the messaging tools never write to or expose the files bucket. The single crossing is read-only and explicit: `ses_send_email` may *read* a `shared/` object to attach it (`Attachments[].RawContentKey`), which costs the caller `msg/read` on top of `msg/email:send`. The media bucket stays off-limits to the email path.
  ...
  "Content": {
    "Simple": {
      ...
      "Attachments": [ { "FileName": "string", "ContentType": "string", "RawContent": "base64",
                         "RawContentKey": "shared/... files-bucket key, instead of RawContent",
                         "ContentDisposition": "ATTACHMENT | INLINE", "ContentId": "Content-ID, with or without angle brackets; a cid: reference resolves to it, see below",
                         "ContentTransferEncoding": "BASE64 | QUOTED_PRINTABLE | SEVEN_BIT",
                         "ContentDescription": "string" } ]
    }
    // "Raw": { "Data": "base64 MIME" }  -- alternative to Simple, or
    // "Raw": { "DataKey": "shared/... files-bucket key holding the whole MIME message" }
  },
...
Server-injected: `ConfigurationSetName` (from env) and a default `ReplyToAddresses` (`esparza.gabriel@gmail.com`, overridable) because the sender domain hosts no mailboxes. Rejected if present: `FromEmailIdentityArn`, `FeedbackForwardingEmailAddress`, `ListManagementOptions`. Exactly one of `Content.Simple` / `Content.Raw`. For `Raw`, the server parses the MIME `From` header and enforces the same sender allow-list, and rejects decoded messages larger than `EmailMaxRawBytes` (default 10 MB); the raw ladder reports as four decisions (`raw_base64`, `raw_size`, `raw_mime`, `sender_allow_list`) so a refusal names the stage that failed. A raw message may instead be named by key — exactly one of `Raw.Data` / `Raw.DataKey`, the latter a `shared/…` files-bucket object holding the *whole* MIME message — because inlining costs base64 twice (a 75 KB image is ~100 KB as a body part and ~156,000 characters once the message is base64-encoded again for `Data`, which a caller emitting that string token by token cannot reach). `DataKey` reads on the same path as `Attachments[].RawContentKey`, requires `msg/read` as well, and runs the identical ladder from `raw_size` on; `raw_base64` is absent because nothing was base64-encoded, and `sender_allow_list` moves *after* the read because the `From` header lives in the fetched object. The cheap guardrails and the rate limiter still refuse before any S3 read. `Simple` attachments spend the same `EmailMaxRawBytes` budget (combined decoded bytes, guardrails `attachment_base64` and `attachment_size`). `ContentDisposition: INLINE` — or a `ContentId` with no disposition at all — embeds the image: SES assembles `Simple` attachments under a `multipart/mixed` root, where a `cid:` reference never resolves, so for those sends **the server assembles the message itself** ([`docs/plans/email-inline-mime.md`](plans/email-inline-mime.md)), putting the HTML part and the inline parts as siblings inside a `multipart/related`, and sends it as `Content.Raw`. The `ContentId` may be written with or without angle brackets (`chart` or `<chart>`), and a **bare id is qualified to `chart@<sender-domain>`** in the `Content-ID` header and in every `cid:` reference to it in the HTML — `Content-ID` is a msg-id, whose grammar requires the `@`, and Gmail silently degrades a bare id to an ordinary attachment (field A/B, 2026-09-01); an id supplied with an `@` is used exactly as given and the HTML must reference that full form. Four guardrails cover what SES used to check for us on that path (`attachment_fields`, `inline_content_id`, `inline_needs_html`, `inline_cid_refs`), and `assembled_size` meters the finished message against SES's own 40 MB ceiling — deliberately not the 10 MB attachment budget, which keeps metering the decoded bytes so nothing that sends today is newly refused. Everything with no inline attachment is untouched: it goes out as `Content.Simple` and SES assembles it. **Breaking change:** a `DryRun` of an inline send now echoes `WouldCall.Content.Raw` instead of `WouldCall.Content.Simple.Attachments`, because that is the exact call made. An attachment may instead name an object the server already holds — `RawContentKey: "shared/…"` from `files_put_object` or `files_list_objects` — and the server reads it from the files bucket and attaches the bytes itself, so nothing binary passes through the caller's context; that path additionally requires `msg/read` (§5.1 rule 7), refuses keys outside `shared/`, refuses an object whose expiry has passed but which the daily cleanup has not deleted yet, and checks the object's size against the attachment budget before downloading it. Resolved references then run the identical size guardrail and digest path as inline bytes. Both paths return `ServerMetadata.content_digests` — `{ "part": "raw" | "attachment[<i>]:<FileName>" | "assembled", "bytes": <decoded length>, "sha256": "<hex>" }` per binary part, identical on a `DryRun` and on the real send — so the caller can prove the bytes arrived intact before and after sending. `raw` is a caller-supplied `Content.Raw` however it arrived, inline or by `DataKey`; `assembled` is the message the server built, reported alongside the per-attachment digests rather than instead of them. `ServerMetadata.mime_structure` describes the message that will actually be sent — a **flat** list of parts, one entry per part (`path` `"1.2.1"`, `depth`, `content_type`, `disposition`, `content_id`, `filename`, encoded `bytes`), a reader rebuilding the tree from the paths if it wants one — for a message the server assembled and for a caller-supplied `Content.Raw`, on a `DryRun` and on the real send alike. Bodies are never included. A `Simple` send has no entry: SES assembles it and the server never sees the tree. It is flat because `jsonschema.For` refuses a named recursive type and the tool's output schema is inferred at registration, so a nested `Parts []Part` would panic every tool registration for the sake of a diagnostic field.
...
| `DryRun` (counts against the rate limit) | available | available | per call |
...
A `DryRun` runs the same guardrails as a real send, the rate limiter included, so **a dry run draws real rate-limit budget** even though nothing is sent. That is deliberate: a free dry run would make the per-tool limit probe-able instead of a budget.
...
- ~~Verify SES sender identity(ies) and request production access (exit sandbox) for `prod`.~~ **Done 2026-08-30:** production access granted (case `178750837400500`); the sandbox is lifted for the whole account-region, so both stages send to unverified recipients subject only to the `RecipientAllowList` guardrail (off in prod, on in dev).
...
| --- | --- | --- |
| 2026-09-01 | **A bare `Content-ID` is qualified, and `ServerMetadata` is an open envelope.** From the first production use after v1.2.0. (1) RFC 2045 defines `Content-ID` as a *msg-id* (`id-left "@" id-right`, the `@` not optional) and Gmail enforces it: `ContentId: weekend-chart` was accepted, delivered, and silently degraded to an ordinary attachment, while `weekend-chart@gabriel-esparza.com` rendered inline. `internal/mimebuild` now qualifies a bare id to `id@<sender-domain>` in the header **and** in every `cid:` reference to it in the HTML — RFC 2392 resolves the URL against the full Content-ID, so qualifying one without the other only moves the failure; an id supplied with an `@` is used as given. (2) Every tool carrying `ServerMetadata` declares an output schema in which `ServerMetadata` permits **additional properties**, because adding `mime_structure` in v1.2.0 made a *successful* send fail the claude.ai connector's cached-schema validation ("must NOT have additional properties"), discarding the response and the `MessageId` in it — the caller saw an error for a delivered message and a retry would have sent it twice. **Schema evolution is a compatibility contract:** `ServerMetadata` changes are additive-only, clients must tolerate unknown fields, and only the envelope is open — every field that exists stays fully described. Documented in the same pass rather than changed: `DryRun` draws real rate-limit budget (deliberate — a free dry run makes the limit probe-able), `ReplyToAddresses` defaults to the server's configured owner address and not to anything derived from the message, and `WouldCall` echoes the whole AWS SDK input struct, so it shows fields the input schema deliberately rejects (`Template`, `Headers`, `Feedback*`, `ListManagementOptions` — §5.3) | The client report in [`docs/plans/email-inline-mime.md`](plans/email-inline-mime.md) §10 (EC-30..EC-32), which also settles half of that plan's §8 acceptance criterion: the qualified id rendered inline in Gmail, the first confirmation from a real client that the assembled `multipart/related` path works at all; Apple Mail and Outlook stay open. Both defects were invisible from the server — the send succeeded, the digests matched, and the reported part tree was correct in each case. One failed in the recipient's client and the other in the caller's, which is the same lesson as the original inline defect: what the API accepted was never the thing being claimed |
| 2026-09-01 | **The assembled message is observable, and a raw message can be sent by key.** `ServerMetadata.mime_structure` returns the MIME part tree of the message `ses_send_email` is about to send — one flat entry per part (path, depth, content type, `Content-ID`, disposition, filename, encoded size), never a body — on a `DryRun` and on the real send, for a message the server assembled *and* for a caller-supplied `Content.Raw`, which is walked with `mimebuild.Walk` under conservative bounds (depth 10, 200 parts) because those bytes are untrusted; a walk that fails omits the field rather than failing the send. It is **flat**, not nested: `jsonschema.For` errors on a named recursive type and `sendEmailOutputSchema` turns that into a `panic` inside `NewServer`, so a `Parts []Part` field would take down every tool registration, `cmd/gendocs`, and the Lambda cold start for a diagnostic. `Content.Raw.DataKey` gives a whole raw MIME message the escape hatch `Attachments[].RawContentKey` already gave one attachment: exactly one of `Data` / `DataKey`, `msg/read` as well as `msg/email:send`, the same expiry and size checks, and the same ladder from `raw_size` on | Both come from the client report in `docs/plans/email-inline-mime.md` (R3 and R2). Without the structure, an image that does not render cannot be told apart from wrong parameters or a client quirk except by sending real mail to a real person and asking what they saw. Without `DataKey`, `Raw` is unreachable for an LLM caller at any realistic image size: a 74,601-byte PNG becomes ~156,400 characters of base64 in one string parameter, ~1.78× the original binary before the HTML is counted. `sender_allow_list` necessarily moves after the read on the `DataKey` path — the `From` header is inside the object — but the recipient guardrails and the rate limiter still refuse before any S3 read, so a throttled caller cannot drive bucket reads |
| 2026-09-01 | **The server assembles `multipart/related` for inline images.** When a `Simple` attachment is `INLINE` (or carries a `ContentId` with no disposition), `ses_send_email` builds the whole MIME message itself with `internal/mimebuild` — HTML part and inline parts as siblings inside a `multipart/related`, ordinary attachments outside it — and sends it as `Content.Raw`, so a `cid:` reference finally resolves. Four guardrails take over the string checks SES performed for us on that path, and `assembled_size` meters the finished message against SES's 40 MB ceiling rather than against the 10 MB attachment budget (base64 inflates by 4/3, so reusing 10 MB would refuse sends that work today). **Breaking change:** a `DryRun` of an inline send now echoes `WouldCall.Content.Raw` instead of `WouldCall.Content.Simple.Attachments`, because that is the exact call made | Everything else keeps SES's own `Simple` assembly, deliberately: the blast radius is then exactly the messages that are broken today, there is no regression for the ones that are not, and the revert is one condition. `Reply-To` moves into the message and off the API parameter (two of them violate RFC 5322 §3.6), while `Bcc` stays on `Destination.BccAddresses` and never becomes a header |
| 2026-09-01 | **Inline (CID) images do not render, and the claim that they did was wrong.** SES `SendEmail` with `Content.Simple` assembles attachments under a root `multipart/mixed`, but a `cid:` reference resolves only when the HTML part and the image part are siblings inside a `multipart/related`; `ContentDisposition: INLINE` plus a `ContentId` is therefore accepted and delivered but arrives as an ordinary attachment. Confirmed by AWS's own [re:Post thread](https://repost.aws/questions/QUfip3VbVIT1-1tEenZyBzSg/email-attachments-in-ses-v2-incorrect-inline-documentation-and-mime-implementation) (Sept 2025), whose recommended workaround is raw MIME. Corrected now: §5.3, the `ses_send_email` tool description, `internal/schemas`, `docs/server.md`, and `docs/files.md` all say so, and the server emits a non-blocking `inline_not_rendered` guardrail decision on any `INLINE`/`ContentId` attachment. `Content.Raw` with a hand-built `multipart/related` is the path that works; server-side assembly of that message is planned in `docs/plans/email-inline-mime.md` | A client reported three sends (two content types, both `ContentId` spellings) accepted and delivered without rendering, with `content_digests` proving the bytes arrived intact. The claim came from `email-composition.md` EC-1, which inferred SES's container choice from the SDK struct and was never verified against a real delivery — the e2e test only ever did a `DryRun`. For an LLM caller the tool description is the entire specification, so a wrong one steers callers away from what does work |
| 2026-08-31 | **Email attach-by-reference:** a `ses_send_email` attachment may name a files-bucket object (`Attachments[].RawContentKey`) instead of carrying inline base64; the server heads it (expiry, then size against the attachment budget) before reading it, and the fetched bytes run the same guardrail and digest path as inline ones. §5.1 rule 7 amended accordingly — the crossing is **read-only and files-bucket-only**; the messaging tools still never write to or expose that bucket, and the media bucket stays off-limits to email. The path demands `msg/read` **in addition to** `msg/email:send`, because reading a shared object is a files-store read and an email-only token must not be able to exfiltrate one. Also recorded from PR2: the go-sdk infers Go `[]byte` as a JSON *array* while `encoding/json` emits a base64 *string*, so the SDK's own output validation had failed every `ses_send_email` `DryRun` carrying binary content since M2 (EC-8); `ses_send_email` now declares an explicit output schema | The client gap report — "the email tool and the file store live on the same server but can't reference each other, so any binary content has to round-trip through the model's context as inline base64"; plan, findings, and PR split in `docs/plans/email-composition.md` |
| 2026-08-30 | **Apex redirect stack** (`infra/apex-site.yaml`, `us-east-1`, one-off, deployed manually after a staging rehearsal on `staging.gabriel-esparza.com`): ACM cert for `gabriel-esparza.com` + `www` SAN and a CloudFront distribution whose viewer-request function 301s every request to `https://mcp.gabriel-esparza.com/`; the root zone's legacy site `A` record and its `SiteIpAddress` parameter are removed, the `www` CNAME chains to the new apex alias | Toll-free carrier verification denied the registration 2026-08-25 for "Company Verification Failed": the apex answered on neither port 80 nor 443, so the company URL could not be verified. Cheapest live web presence that adds no content to maintain |
