...
| Field | Value |
| --- | --- |
| Status | Executed 2026-08-22 — M0 complete, M1 exit criteria met on dev; outcomes in PRD Appendix C |
| Date | 2026-08-22 |
...
- Local toolchain: `gh` 2.98.0 ✓, `jq` 1.7.1 ✓, AWS CLI 2.25.9 ✓, Python 3.13.5 (Homebrew) ✓, AWS credentials = `AdministratorAccess` SSO role on the target account ✓. `uv` is **0.6.11 (2025-03)** — upgrade before scaffolding (`brew upgrade uv`), both for Python 3.13 lockfile behavior and the stale-cache issue from F4.
...
3. **Run** `GH_OWNER=gesparza8138 AWS_ACCOUNT_ID=<account-id> ./scripts/setup-github.sh --create` — creates the private repo, pushes the scaffold, then applies settings, environments, variables, Actions policy, labels, and finally the two rulesets (ordering already correct in the script). Outward action → gate G3 first.
...
- `infra/bootstrap.yaml`: `AWS::IAM::OIDCProvider` (GitHub), `aws-messaging-mcp-deploy-dev` role (trust `repo:gesparza8138/aws-messaging-mcp:environment:dev`), `...-deploy-prod` (trust `environment:prod`), artifact bucket `aws-messaging-mcp-artifacts-<account-id>`, CFN service role. Dev role includes the scoped `wafv2` IP-set permission from F7 so the R7 decision isn't blocked on a later bootstrap change.
